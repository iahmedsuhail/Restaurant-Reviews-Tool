...
class CompareComponent extends React.Component {
  ...
  UNSAFE_componentWillMount() {
    ...
    fetch(
      `${"https://cors-anywhere.herokuapp.com/"}https://maps.googleapis.com/maps/api/place/details/json?key=${GOOGLE_PLACES_API_KEY}&place_id=${googleCompareId}&type=restaurant&fields=photo,review,formatted_address,formatted_phone_number,name,rating,price_level`,
      {
        mode: "cors",
      }
    )
        ...
        this.setState(
          ...
          function () {
            if (this.state.gpPhotoReference !== "") {
              //Get Image jpg
              fetch(
                `${"https://cors-anywhere.herokuapp.com/"}https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=${
                  this.state.gpPhotoReference
    ...
    axios
      .get(
        `${"https://cors-anywhere.herokuapp.com/"}https://api.yelp.com/v3/businesses/${yelpCompareId}`,
        {
          headers: {
    ...
    axios
      .get(
        `${"https://cors-anywhere.herokuapp.com/"}https://api.yelp.com/v3/businesses/${yelpCompareId}/reviews`,
        {
          headers: {
            Authorization: `Bearer ${YELP_PLACES_API_KEY}`,
            "X-Requested-With": "XMLHttpRequest",
    ...
    fetch(
      `${"https://cors-anywhere.herokuapp.com/"}https://developers.zomato.com/api/v2.1/restaurant?res_id=${zomatoCompareId}`,
      {
        mode: "cors",
        headers: {
    ...
    fetch(
      `${"https://cors-anywhere.herokuapp.com/"}https://developers.zomato.com/api/v2.1/reviews?res_id=${zomatoCompareId}`,
      {
        mode: "cors",
        headers: {
          Accept: "application/json",
    ...
    if (
      ...
      return (
        <div className="App">
          ...
          <h3>
            Please select 1 - 3 reviews for comparison, and access this page
            again
          </h3>
        </div>
      );
              ...
              <Grid style={{ gridColumn: "1/3" }}>
                {this.state.zomatoName !== "" ? (
                  <div>
                    <h2>Zomato</h2>
                    <ZomatoReviewCard
                      zomatoName={this.state.zomatoName ?? ""}
    ...
    <div>
      <Grid container direction="column" justify="center" alignItems="center">
        <div className="tc review-component  br3 pa3 ma2 dib bw2 shadow-5 sunflower-light mw-75 w-75">
          ...
          <h3>
            Name: {yelpName ?? "Please select a yelp review for comparison"}{" "}
            <br />
            Phone: {yelpPhone ?? ""} <br />
            Address: {yelpAddress ?? ""} <br />
            Webpage:
            <br />
            <a href={yelpWebsite ?? ""}>Link</a>
            <br />
            Price: {yelpPrice ?? ""} <br />
          ...
          <div className="reviewBox">
            {yelpUserReviews != null
              ? yelpUserReviews.map((re, i) => {
                  return (
                    <h3 key={i.toString()} className="review">
                      {yelpUserReviews[i].rating}/5 - {yelpUserReviews[i].text}{" "}
                      <br />
                    </h3>
                  );
                })
    ...
    <div>
      <Grid container direction="column" justify="center" alignItems="center">
        <div className="tc review-component  br3 pa3 ma2 dib bw2 shadow-5 sunflower-light mw-75 w-75">
          ...
          <h3>
            Name: {gpName ?? "Please select a google review for comparison"}{" "}
            <br />
            Phone: {gpPhone ?? ""} <br />
            Address: {gpAddress ?? ""} <br />
            Webpage:
            <br />
            <a href={gpWebsite ?? ""}>Link</a> <br />
            Price: {gpPrice ?? ""} <br />
            Rating: {gpRating ?? ""} <br />
            Individual Reviews: <br />
          </h3>
          <div className="reviewBox">
            {gpUserReviews != null
              ? gpUserReviews.map((re, i) => {
                  return (
                    <h3 key={i.toString()} className="review">
                      {gpUserReviews[i].rating}/5 - {gpUserReviews[i].text}{" "}
                      <br />
                    </h3>
                  );
                })
    ...
    <div>
      <Grid container direction="column" justify="center" alignItems="center">
        <div className="tc review-component  br3 pa3 ma2 dib bw2 shadow-5 sunflower-light mw-75 w-75">
          ...
          <h3>
            Name: {zomatoName ?? "Please select a zomato review for comparison"}{" "}
            <br />
            Address: {zomatoAddress ?? ""} <br />
            Webpage:
            <br />
            <a href={zomatoWebsite ?? ""}>Link</a>
            <br />
          ...
          <div className="reviewBox">
            {zomatoUserReviews != null
              ? zomatoUserReviews.map((re, i) => {
                  return (
                    <h3 key={i.toString()} className="review">
                      {zomatoUserReviews[i].review.rating}/5 -{" "}
                      {zomatoUserReviews[i].review.review_text} <br />
                    </h3>
                  );
                })
              : ""}
